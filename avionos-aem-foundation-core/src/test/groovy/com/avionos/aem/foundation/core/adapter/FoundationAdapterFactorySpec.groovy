...
class FoundationAdapterFactorySpec extends FoundationSpec {
    ...
    @Shared
        adapterFactory = new FoundationAdapterFactory()

    def "get resource adapter for component resource returns non-null"() {
        setup:
        def resource = resourceResolver.getResource("/")

        expect:
        adapterFactory.getAdapter(resource, ComponentResource)
    }
